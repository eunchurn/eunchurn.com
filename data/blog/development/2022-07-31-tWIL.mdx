...
그렇다. 여기도 빠른 `prisma` 개발에 따라가긴 어려운 모양이다. 약 한달전 Prisma 는 버전 4로 업그레이드가 되었고, 여기는 아직 작업중이라고 한다. [@paljs/nexus : NEXUS**UNKNOWN**TYPE was already defined when updating to prisma 4.0.0](https://github.com/paljs/prisma-tools/issues/273)

그래서 Prisma 버전은 3.15.2로 내려주자.
