...
지난주 tWIL에서 Notion API로 Vercel과 AWS Amplify 모두 배포해보고 얻은 결과는 Vercel은 컨텐츠가 즉각 변경되는 점에 비해 Amplify는 5-10분 정도 걸린다. API를 캐싱해두고 작동하는 것 같은데 이려러고 돈내고 쓰는게 아닐텐데 어디 설정도 찾기 어렵다. 이에 비해 Vercel은 무료잖아? 그렇다고 전세계 CDN에 배포가 안되는 것도 아니고 너무 비교가 된다. 빨리 Amplify를 벗어나고 싶다.
