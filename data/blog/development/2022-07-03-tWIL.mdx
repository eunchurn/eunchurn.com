...
- 우리가 쓸 컴포넌트에 예외처리들이 적용된 컨테이너 컴포넌트를 위한 `getServerSideProps`를 얻을 수 있다. 이 후 프리젠터 컴포넌트 개발에서는 데이터 신경쓸 필요가 없다. 그렇다. 좀 복잡해진다.
- 그리고 known issue가 있다. `getServerSideProps`는 지연이 꽤 큰 편이다. [Slow SSR Rendering](https://github.com/aws-amplify/amplify-hosting/issues/2530) 따라서 Amplify 에서 Nextjs는 추천하지 않겠다.
...
- Custom Resolvers
  ...
  - 복잡한 Custom resolver가 필요한 겨우 Apache VTL 학습 필요. (https://velocity.apache.org/engine/1.7/user-guide.html)
