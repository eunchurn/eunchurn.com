...
그렇다고 우리가 신생하는 오픈 소스에 눈을 돌릴 필요 없다. 언제까지나 오픈 소스로 참여한 코드들은 전세계 개발자의 것(MIT License). 제공하는 오픈 소스 서비스는 마음껏 활용하면서 DevX는 알아서 한땀한땀 만들어가며 챙겨야할 필요가 있다. 클라우드는 대기업들이 잘 구축해놓은 성공한 인프라만 사용하도록 하자. IaaS로만 말이다. 컨테이너와 DB를 얻어서 사용하고 데이터는 클라우드에게 내어주지 말자.
