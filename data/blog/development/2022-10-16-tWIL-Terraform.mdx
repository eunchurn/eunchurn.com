...
Terraform 모듈을 사용하여 구성하였으며, 네트워크부터 시작하면
...
ECS의 Service에서 삽질을 많이했다. 즉, `CODE_DEPLOY`를 Deployment Controller로 사용할 때 다른 인프라를 수정할 때도 배포 오류가 발생한다. ECS Service를 CodeDeploy로 배포한다고 정의했기 때문에 직접 인프라 수정이 안되는 것이다.
...
변경점을 무시하라는 lifecycle을 정의해 주어야 배포가 가능했다. 물론 ECS가 변경이 되면 클러스터와 서비스를 모두 다시 생성해 주어야 한다. 만약 Production 운영중이라면, 문제가 된다.
...
일반적인 `CodeDeploy` best practice에서 `load_balancer`에 `prod_traffic_route`을 설정하는데 삽질을 많이 했다. Blue 로드밸런서를 붙여주면 CodeDeploy는 배포과정에서 Blue에서 자동으로 Green으로 변경해주며, ALB 설정도 Green으로 변경해준다. 이 사실을 모르고 두개를 붙여보기도 하고, ALB에 여러 타겟을 붙여보기도 하였다.
...
여기서 코드가 너무 길어서 각 모듈에 대한 코드는 닫아놓았다.
