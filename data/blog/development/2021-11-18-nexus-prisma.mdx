...
model TestUser {
  ...
  /// 계정정보 (password, salt), will be deprecated
  accountInfo                       Json                   @default("{}")
  /// 사용자 비밀번호 Hash (accountInfo -> password)
  password                          String?
  ...
  /// 사용자 회원가입 타입
  type                              Users_type?            @default(email)
  /// 소셜로그인의 경우 accountId
  accountId                         String?                @db.VarChar(200)
  /// 사용자 상태 (정상, 휴면, 탈퇴등)
  status                            Users_status           @default(general)
  /// 탈퇴 시각
  deletedAt                         DateTime?              @db.Timestamp(0)
  /// 탈퇴 유형
  deleteType                        Users_deleteType?
  /// 탈퇴 사유
  deleteDescription                 String?                @db.VarChar(200)
  /// 사용자 지역
  Cities                            Cities?                @relation(fields: [cityId], references: [id])
  /// 추천인
  Referrer                          CashUser?              @relation("UsersToUsers_referrerId", fields: [referrerId], references: [id])
  /// 본인 인증 데이터
  ...
  UserMemos                         UserMemos[]
  /// 피추천인 유저
  other_Users                       CashUser[]             @relation("UsersToUsers_referrerId")
  /// 사용자 이벤트
  ...
  /// 사용자 매장 미션 연결
  Users_StoresMissions              Users_StoresMissions[]
  /// 사용자 매장 설문조서 연결
  ...
  /// AD.Fi MAC 사용자 연결
  UniqueUser                        UniqueRawUser[]
  /// 사용자 선호 카테고리
  UserCategories                    CashUserCategory[]
  /// 유저 유입 경로
  referral                          ServiceType            @default(CASH_FI)

  @@index([cityId], name: "Users_Cities_id_fk")
  @@index([referrerId], name: "Users_Users_id_fk")
}
```
...
export const TestUserType = objectType({
  ...
  definition(t) {
    ...
    t.field(TestUser.cityId);
    t.field(TestUser.isMarried);
    t.field(TestUser.referralCode);
    t.field(TestUser.deviceId);
    t.field(TestUser.point);
    t.field(TestUser.referrerId);
    t.field(TestUser.isMarketing);
    t.field(TestUser.type);
    t.field(TestUser.accountId);
    t.field(TestUser.status);
    t.field(TestUser.deletedAt);
    t.field(TestUser.deleteType);
    t.field({
