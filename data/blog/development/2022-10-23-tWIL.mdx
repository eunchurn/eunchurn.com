...
이제 Lambda 인프라를 설정한다. `archive_file` 모듈로 배포된 위치(`dist`)를 `function.zip`으로 압축하고, `aws_lambda_layer_version` 모듈은 스크립트를 통해 압축한 `layers.zip`을 업로드한다. `aws_lambda_function`은 앞서 `archive_file` 모듈로 압축한 파일을 담도록 설정하고 `handler` 어트리뷰트에 해당 함수이름 `index.handler`로 설정한다. 그리고 Lambda에는 Lambda layer를 연결한다. `layers = [aws_lambda_layer_version.dependency_layer.arn]`
