...
이러한 방법을 응용한다면 주로쓰는 사운드에서 필요한 RANK부분의 소리를 제거하고 다른 소리를 채워 넣을 수도 있다. 중요한점은 교유벡터가 서료 직교하는 상태일때여야 한다. 참고로 사용된 프로그램은 MATLAB으로 만들었고 좀더 개발하고 쓸모가 있어지면 공개할 예정이다. (시간이 많이 지난 요즘, 2022년에 와서야 Python으로 다시 구현하였다. <https://github.com/eunchurn/signal-decomposition> 하지만 MATLAB버전은 GUI가 있어서 샘플마다 리스닝이 가능했지만, Python은 어떻게 해야할지...)
