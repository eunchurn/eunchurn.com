...
import type { Authors, Blog, History, Research } from 'contentlayer/generated'
...
export async function generateMetadata(props: {
  ...
  const history = allResearch.find((p) => p.slug === params.pageId)
  if (!history) {
    return
  }
  const { image } = history
...
export const generateStaticParams = async () => {
  return allResearch.map((p) => ({ slug: p.slug.split('/').map((name) => decodeURI(name)) }))
}
...
export default async function Page(props: { params: Promise<{ pageId: string }> }) {
  ...
  const sortedCoreContents = allCoreContent(allResearch)
  ...
  const post = allResearch.find((p) => p.slug === pageId) as Research
  const mainContent = coreContent(post as unknown as History)
  // add ul className
